...
class RAGService:
    ...
    # Verified Nepali ↔ English animal name reference used in list prompts
    _NEPALI_ANIMAL_NAMES = (
        "VERIFIED NEPALI ANIMAL NAMES (use ONLY these — never invent or mix languages):\n"
        "बाघ = Bengal Tiger | गैंडा = One-horned Rhinoceros | हात्ती = Asian Elephant\n"
        "गौर = Gaur (Indian Bison) | चितुवा = Leopard | भालु = Sloth Bear\n"
        "हिरण = Spotted Deer (Chital) | सम्भर = Sambar Deer | बँदेल = Wild Boar\n"
        "मगर = Mugger Crocodile | घडियाल = Gharial | अजिंगर = Rock Python\n"
        "मयूर = Peacock | सारस = Sarus Crane | खरमुजुर = Bengal Florican\n"
        "गिद्ध = Vulture | बाज = Eagle/Hawk | माछा चरा = Kingfisher\n"
        "डल्फिन = Gangetic Dolphin | माहासिर = Mahseer Fish\n"
        "CONSERVATION STATUS IN NEPALI:\n"
        "गम्भीर संकटग्रस्त = Critically Endangered | संकटग्रस्त = Endangered\n"
        "सुरक्षा आवश्यक = Vulnerable | न्यून चिन्ता = Least Concern\n"
    )
    ...
    def _prompt_list(self):
        return PromptTemplate(
            template=(
                ...
                + self._NEPALI_ANIMAL_NAMES + "\n"
                "STRICT RULES:\n"
                "- Start IMMEDIATELY with the first bullet. No introduction.\n"
                "- List MAXIMUM 6 items.\n"
                "- PRIORITIZE endangered, threatened, and vulnerable species first.\n"
                ...
                "- Never write 'here are' or any preamble. Never add closing remarks. Just stop after 6 items.\n"
                "IF THE QUESTION IS IN ENGLISH:\n"
                "  Format: • English Name (Nepali Name) - conservation status + one brief fact.\n"
                "  Example: • Bengal Tiger (बाघ) - Endangered; around 120 tigers live in Chitwan.\n"
                "IF THE QUESTION IS IN NEPALI:\n"
                "  Format: • नेपाली नाम (English Name) - संरक्षण स्थिति + एक तथ्य।\n"
                "  Example: • बाघ (Bengal Tiger) - संकटग्रस्त; चितवनमा करिब १२० बाघ छन्।\n"
                "  CRITICAL FOR NEPALI: Use ONLY pure Nepali words — NEVER mix English letters inside a Nepali word.\n"
                "  WRONG: 'मुगर crocodile' or 'कrocodile' — RIGHT: 'मगर (Mugger Crocodile)'\n"
                "  Use the verified Nepali names from the table above. If unsure of the Nepali name, write the English name in brackets.\n\n"
                "Context:\n{context}\n\n"
                "Chat history:\n{chat_history}\n\n"
                "Question: {question}\n\nAnswer:"
            ),
    ...
    def _prompt_bare(self):
        return PromptTemplate(
            template=(
                ...
                "IF THE QUESTION IS IN ENGLISH:\n"
                "  Format: • English Name (Nepali Name)\n"
                "IF THE QUESTION IS IN NEPALI:\n"
                "  Format: • नेपाली नाम (English Name)\n"
                "  CRITICAL FOR NEPALI: NEVER mix English letters inside a Nepali word.\n"
                "  WRONG: 'मुगर crocodile' — RIGHT: 'मगर (Mugger Crocodile)'\n"
                "  Use the verified Nepali names from the table above.\n\n"
                "Context:\n{context}\n\n"
                "Chat history:\n{chat_history}\n\n"
                "Question: {question}\n\nAnswer:"
            ),
